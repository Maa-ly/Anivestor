...
contract WhiteList{
    ...
    constructor () {
     ...
     function verifyWhiteList(address Add) external view  returns(bool) {
        require(privateWhitelist[Add] == true || publicWhitelist[Add] == true, "revert_not_in_public_or_private_whitelist");
        return true;
     }

     function isWhiteList() external view returns(bool){
        require(privateWhitelist[msg.sender] == true || publicWhitelist[msg.sender] == true, "revert_not_in_public_or_private_whitelist");
        return true;
     }
