...
contract DeployAnivestor is Script {
    ...
    function run() external returns (FarmerRegistration, WhiteList, MarketPlace, Borrow, HelperConfig) {
      ...
      //   address farmerWhiteList = whiteListDeployer.getFarmerWhiteList(msg.sender);
        ...
        MarketPlace marketPlace = new MarketPlace(URI, address(whiteList), address(farmer), usdt);

        Borrow borrow = new Borrow(address(whiteList), address(farmer), usdt, wbtc, wbtcUsdPriceFeed);
        // wethUsdPriceFeed, wbtcUsdPriceFeed
        // wbtc/usd right?

        vm.stopBroadcast();

        return (farmer, whiteList, marketPlace, borrow, helperConfig);
    }
}
